...
# Update user preferences with proper enum values
...
try {
    ...
    Write-Host "   - Preferred categories: $($updateResponse.preferredCategories -join ', ')" -ForegroundColor Gray
    Write-Host "   - Min busyness: $($updateResponse.minBusyness)" -ForegroundColor Gray
    Write-Host "   - Preferred vibes: $($updateResponse.preferredVibes -join ', ')" -ForegroundColor Gray
} catch {
